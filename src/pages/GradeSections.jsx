...
const inputClass =
  "h-11 w-full rounded-md border border-slate-200 bg-white px-3 text-sm outline-none transition focus:border-cyan-500 focus:ring-4 focus:ring-cyan-50";
const primaryButton =
  "inline-flex h-11 w-max shrink-0 items-center justify-center gap-2 whitespace-nowrap rounded-md bg-slate-950 px-4 text-sm font-bold text-white transition hover:bg-cyan-600";

const Status = ({ active }) => (
  <span
    className={`rounded-full px-2.5 py-1 text-xs font-bold ${active ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}
  >
...
export default function GradeSections() {
  ...
  const load = async () => {
    try {
      setData((await api.get("/api/academic-setup")).data);
    } catch (error) {
  ...
  const filteredRows = useMemo(() => {
    ...
    if (activeView === "academicUnits")
      return data.academicUnits.filter((row) =>
        `${row.code} ${row.name} ${row.type} ${row.education_level} ${row.parent?.name || ""}`
          .toLowerCase()
          .includes(term),
      );
    if (activeView === "academicPrograms")
      return data.academicPrograms.filter((row) =>
        `${row.code} ${row.name} ${row.program_type} ${row.academic_unit?.name || ""}`
          .toLowerCase()
          .includes(term),
      );
    if (activeView === "schoolYears")
      return data.schoolYears.filter((row) =>
        `${row.name} ${row.start_date || ""} ${row.end_date || ""}`
          .toLowerCase()
          .includes(term),
      );
    if (activeView === "gradeLevels")
      return data.gradeLevels.filter((row) =>
        `${row.name} ${row.academic_unit?.name || ""} ${row.academic_program?.name || ""}`
          .toLowerCase()
          .includes(term),
      );
    return data.sections.filter((row) =>
      `${row.name} ${row.grade_level?.name || ""} ${row.grade_level?.academic_unit?.name || ""} ${row.grade_level?.academic_program?.name || ""} ${row.school_year?.name || ""}`
        .toLowerCase()
        .includes(term),
    );
  ...
  return (
    <div className="space-y-6" data-aos="fade-up">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Grade & Sections</h1>
        <p className="mt-1 text-sm text-slate-500">
          Manage your academic structure and teacher assignments.
        </p>
      </div>

      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        {cards.map(({ key, label, count, icon: Icon, color, bg }) => (
          <button
            key={key}
            type="button"
            onClick={() => selectView(key)}
            className={`flex items-center gap-4 rounded-xl border bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${activeView === key ? "border-cyan-500 ring-4 ring-cyan-50" : "border-slate-200"}`}
          >
            <span
            ...
            <span>
              ...
              <span className="mt-1 block text-3xl font-black text-slate-900">
                {count}
              </span>
            </span>
          </button>
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col gap-4 border-b border-slate-200 p-5 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            >
              ...
              {showAddForm ? "Close" : `Add ${activeCard.singular}`}
            </button>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          {activeView === "academicUnits" && (
            <AcademicUnitTable rows={filteredRows} onDelete={remove} />
          )}
          {activeView === "academicPrograms" && (
            <AcademicProgramTable rows={filteredRows} onDelete={remove} />
          )}
          {activeView === "schoolYears" && (
            <SchoolYearTable rows={filteredRows} onDelete={remove} />
          )}
          {activeView === "gradeLevels" && (
            <GradeLevelTable rows={filteredRows} onDelete={remove} />
          )}
          {activeView === "sections" && (
            <SectionTable
              rows={filteredRows}
...
const UnitForm = ({ value, setValue, units, onSubmit }) => (
  <form
    onSubmit={onSubmit}
  ...
  >
    ...
    <select
      ...
      onChange={(e) => setValue({ ...value, education_level: e.target.value })}
    >
      <option value="basic">Basic Education</option>
      <option value="higher_education">Higher Education</option>
    </select>
    <button className={primaryButton}>
      <FiPlus /> Save Academic Unit
    </button>
  </form>
);

const ProgramForm = ({ value, setValue, units, onSubmit }) => (
  <form
    onSubmit={onSubmit}
  ...
  >
    <select
      ...
      onChange={(e) => setValue({ ...value, academic_unit_id: e.target.value })}
    >
    ...
    <select
      ...
      onChange={(e) => setValue({ ...value, program_type: e.target.value })}
    >
    ...
    <button className={primaryButton}>
      <FiPlus /> Save Program / Track
    </button>
  </form>
);

const YearForm = ({ value, setValue, onSubmit }) => (
  <form onSubmit={onSubmit} className="grid gap-3 md:grid-cols-4">
    ...
    />
    <button className={primaryButton}>
      <FiPlus /> Save School Year
    </button>
  </form>
);

const GradeForm = ({ value, setValue, data, onSubmit }) => {
    ...
    <form onSubmit={onSubmit} className="grid gap-3 md:grid-cols-5">
      <select
        ...
        onChange={(e) =>
          setValue({
            ...value,
            academic_unit_id: e.target.value,
            academic_program_id: "",
          })
        }
      >
      ...
      <select
        ...
        onChange={(e) =>
          setValue({ ...value, academic_program_id: e.target.value })
        }
      >
      ...
      <input
        ...
        onChange={(e) => setValue({ ...value, sort_order: e.target.value })}
      />
      <button className={primaryButton}>
        <FiPlus /> Save Grade Level
      </button>
    </form>
  );
};

const SectionForm = ({ value, setValue, data, onSubmit }) => (
  <form onSubmit={onSubmit} className="grid gap-3 md:grid-cols-5">
    <select
      className={inputClass}
      required
      value={value.grade_level_id}
      onChange={(e) => setValue({ ...value, grade_level_id: e.target.value })}
    >
    ...
    <select
      ...
      onChange={(e) => setValue({ ...value, school_year_id: e.target.value })}
    >
...
const AcademicUnitTable = ({ rows, onDelete }) => (
  <table className="w-full text-left text-sm">
    <thead className="bg-slate-50 text-xs uppercase tracking-wider text-slate-500">
      <tr>
        <th className="p-4">Academic Unit</th>
        <th className="p-4">Parent</th>
        <th className="p-4">Type</th>
        <th className="p-4">Education Level</th>
        <th className="p-4">Programs / Grades</th>
    ...
    <tbody>
      {rows.map((row) => (
        <tr key={row.id} className="border-t border-slate-100">
          <td className="p-4 font-bold text-slate-900">
            {row.name}
            <div className="text-xs font-normal text-slate-500">{row.code}</div>
          </td>
          <td className="p-4">{row.parent?.name || "—"}</td>
          ...
          <td className="p-4">
            {row.programs_count} / {row.grade_levels_count}
          </td>
          <td className="p-4">
      ...
      {!rows.length && <EmptyRow columns={7} label="academic units" />}
    </tbody>
  </table>
);

const AcademicProgramTable = ({ rows, onDelete }) => (
  <table className="w-full text-left text-sm">
    <thead className="bg-slate-50 text-xs uppercase tracking-wider text-slate-500">
      <tr>
        ...
        <th className="p-4">Academic Unit</th>
        <th className="p-4">Type</th>
        <th className="p-4">Grade Levels</th>
        <th className="p-4">Status</th>
        <th className="p-4"></th>
      </tr>
    </thead>
    <tbody>
      {rows.map((row) => (
        <tr key={row.id} className="border-t border-slate-100">
          ...
          <td className="p-4">{row.academic_unit?.name}</td>
          <td className="p-4 capitalize">{row.program_type}</td>
          <td className="p-4">{row.grade_levels_count}</td>
          <td className="p-4">
            <Status active={row.is_active} />
          </td>
          <td className="p-4 text-right">
            <DeleteButton
              onClick={() => onDelete("academic-programs", row.id, row.name)}
            />
          </td>
        </tr>
      ))}
      {!rows.length && <EmptyRow columns={6} label="programs or tracks" />}
    </tbody>
  </table>
);

const SchoolYearTable = ({ rows, onDelete }) => (
  <table className="w-full text-left text-sm">
    <thead className="bg-slate-50 text-xs uppercase tracking-wider text-slate-500">
      <tr>
    ...
    <tbody>
      {rows.map((row) => (
        <tr key={row.id} className="border-t border-slate-100">
          ...
          <td className="p-4 text-slate-600">{row.start_date || "—"}</td>
          <td className="p-4 text-slate-600">{row.end_date || "—"}</td>
          <td className="p-4">{row.sections_count}</td>
          <td className="p-4">
            <Status active={row.is_active} />
          </td>
          <td className="p-4 text-right">
      ...
      {!rows.length && <EmptyRow columns={6} label="school years" />}
    </tbody>
  </table>
);

const GradeLevelTable = ({ rows, onDelete }) => (
  <table className="w-full text-left text-sm">
    <thead className="bg-slate-50 text-xs uppercase tracking-wider text-slate-500">
      <tr>
    ...
    <tbody>
      {rows.map((row) => (
        <tr key={row.id} className="border-t border-slate-100">
          <td className="p-4 font-bold text-slate-900">{row.name}</td>
          <td className="p-4 text-slate-600">{row.academic_unit?.name}</td>
          <td className="p-4 text-slate-600">
            {row.academic_program?.name || "—"}
          </td>
          <td className="p-4">{row.sort_order}</td>
          <td className="p-4">{row.sections_count}</td>
          <td className="p-4">
            <Status active={row.is_active} />
          </td>
      ...
      {!rows.length && <EmptyRow columns={7} label="grade levels" />}
    </tbody>
  </table>
);

const SectionTable = ({ rows, teachers, onDelete, onToggleTeacher }) => (
  <table className="w-full text-left text-sm">
    <thead className="bg-slate-50 text-xs uppercase tracking-wider text-slate-500">
      <tr>
    ...
    <tbody>
              ...
              {teachers.length ? (
                teachers.map((teacher) => {
                  ...
                  const name = teacher.staff_profile?.first_name
                    ? `${teacher.staff_profile.first_name} ${teacher.staff_profile.last_name}`
                    : teacher.username;
                  return (
                    <button
                      key={teacher.id}
                      ...
                      className={`rounded px-2 py-1 text-xs font-semibold ${selected ? "bg-cyan-600 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}
                    >
                      {name}
                    </button>
                  );
      ...
      {!rows.length && <EmptyRow columns={6} label="sections" />}
    </tbody>
  </table>
);
